...
const startInquirer = () => {
  inquirer
    ...
    .then((answers) => {
      ...
      if (nextPrompt === "Delete a position or role") {
        deletePosition();
      }
...
const viewDepartments = () => {
  ...
  db.query(sql, (err, rows) => {
    ...
    //     const sql = `SELECT employees.id, first_name, last_name
    //               FROM employees
...
const viewEmployees = () => {
  const sql = `SELECT employees.id, employees.first_name, employees.last_name, positions.title, departments.name AS department, positions.salary, CONCAT(manager.first_name, ' ', manager.last_name) AS manager FROM employees LEFT JOIN positions ON employees.position_id = positions.id LEFT JOIN departments ON positions.department_id = departments.id LEFT JOIN employees manager ON employees.manager_id = manager.id`;
  db.query(sql, (err, rows) => {
    if (err) {
      throw err;
    }
      ...
      db.query(sql, (err, rows) => {
        ...
        return inquirer
          ...
          .then((departmentAnswer) => {
            const department = departmentAnswer.department_id;
  ...
  db.query(sql, (err, rows) => {
    ...
    inquirer
      ...
      .then((employeeAnswer) => {
        const department = employeeAnswer.department;
        const params = [department];
        const sql = `SELECT employees.id, fSELECT employees.id, employees.first_name, employees.last_name, departments.name AS department
        FROM employees
        LEFT JOIN positions ON employees.position_id = positions.id
        LEFT JOIN departments ON positions.department_id = departments.id
        WHERE departments.id = ?irst_name, last_name, departments.name AS department FROM employees LEFT JOIN positions ON employees.positions_id = positions.id WHERE departments.name = ?`;
        db.query(sql, params, (err, rows) => {
          if (err) {
            throw err;
          }
...
//         const department = employeeAnswer.department;
//         const params = [department];
//         const sql = `SELECT employees.id, first_name, last_name, departments.name AS department FROM employees LEFT JOIN positions ON employees.positions_id = positions.id WHERE departments.name = ?`;
//         db.query(sql, params, (err, rows) => {
//           if (err) {
//             throw err;
//           }
...
const deletePosition = () => {
  ...
  db.query(sql, (err, rows) => {
    ...
    inquirer
      ...
      .then((positionAnswer) => {
        const role = positionAnswer.role;
        const params = [role];
